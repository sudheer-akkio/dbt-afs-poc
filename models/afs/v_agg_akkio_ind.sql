...
SELECT
    ...
    -- Credit Card Info as OBJECT (mapped to CreditCards class keys)
    OBJECT_CONSTRUCT(
        ...
        'premium_visa_or_mastercard', CASE WHEN TRY_CAST(attr.CREDIT_CARD_INFO_VISA_SIGNATURE AS INT) < 50 THEN 1 ELSE 0 END,
        'regular_american_express', CASE WHEN TRY_CAST(attr.CREDIT_CARD_INFO_AMEX_USER AS INT) < 50 THEN 1 ELSE 0 END,
        'regular_discover', CASE WHEN TRY_CAST(attr.CREDIT_CARD_INFO_DISCOVER_USER AS INT) < 50 THEN 1 ELSE 0 END,
        'regular_store_or_retail', CASE WHEN TRY_CAST(attr.CREDIT_CARD_INFO_STORE_CC_USER AS INT) < 50 THEN 1 ELSE 0 END,
        'regular_visa_or_mastercard', CASE WHEN TRY_CAST(attr.CREDIT_CARD_INFO_MASTERCARD_USER AS INT) < 50 THEN 1 ELSE 0 END
    ) AS CREDIT_CARD_INFO,
